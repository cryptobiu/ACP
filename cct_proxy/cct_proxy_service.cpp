...
cct_proxy_service::cct_proxy_service(const char * log_category)
: m_logcat(log_category), m_base(NULL), m_tcp(NULL), m_svc_sock(-1), m_conn_sock(-1), m_conn_rd(NULL), m_conn_wr(NULL), m_cc(NULL)
{
...
int cct_proxy_service::serve(const service_t & a_svc, const client_t & a_clnt)
{
	...
	LC.notice("%s: start %lu.%03lu", __FUNCTION__, ts.tv_sec, ts.tv_nsec/1000000);
			...
			if(0 == event_add(sigint_event, NULL))
			{
				if(0 == start_tcp_svc())
				{
					...
					if(NULL != m_tcp)
					{
						...
						{
							LC.info("%s: Running event loop.", __FUNCTION__);
							event_base_dispatch(m_base);
							result = 0;
							event_del(m_tcp);
						}
						else
							LC.error("%s: proxy service tcp event addition failed.", __FUNCTION__);

						event_free(m_tcp);
						m_tcp = NULL;
					}
					else
						LC.error("%s: proxy service tcp event allocation failed.", __FUNCTION__);

					close(m_svc_sock);
					m_svc_sock = -1;
				}
				else
					LC.error("%s: proxy service tcp service start failed.", __FUNCTION__);
				event_del(sigint_event);
			}
			else
				LC.error("%s: proxy service sigint event addition failed.", __FUNCTION__);
			event_free(sigint_event);
			sigint_event = NULL;
		}
		else
			LC.error("%s: proxy service sigint event allocation failed.", __FUNCTION__);

		event_base_free(m_base);
		m_base = NULL;
	}
	else
		LC.error("%s: proxy service event base allocation failed.", __FUNCTION__);

	clock_gettime(CLOCK_REALTIME, &ts);
	LC.notice("%s: stop %lu.%03lu", __FUNCTION__, ts.tv_sec, ts.tv_nsec/1000000);
	return result;
}
...
int cct_proxy_service::start_tcp_svc()
{
	...
	if (0 != bind(m_svc_sock, (const sockaddr *)&service_address, (socklen_t)sizeof(struct sockaddr_in)))
	{
        ...
        return -1;
		 ...
		 LC.debug("%s: socket fd %d bound to [%s:%hu].", __FUNCTION__, m_svc_sock, m_svc.ip.c_str(), m_svc.port);
	...
	if (0 != listen(m_svc_sock, 1))
	{
        ...
        return -1;
		 ...
		 LC.debug("%s: listening with socket fd %d on [%s:%hu].", __FUNCTION__, m_svc_sock, m_svc.ip.c_str(), m_svc.port);
...
{
	LC.info("%s: Breaking event loop.", __FUNCTION__);
    ...
    if(0 <= m_conn_sock)
    {
    	...
    	if(0 == pipe(m_conn_pipe))
    	{
			...
			if(NULL != m_conn_wr)
    		{
				...
				{
		    		LC.debug("%s: proxy client conn write event added.", __FUNCTION__);
					m_conn_rd = event_new(m_base, m_conn_sock, EV_READ|EV_PERSIST, connrd_cb, this);
					if(NULL != m_conn_rd)
					{
						if(0 == event_add(m_conn_rd, NULL))
						{
				    		LC.debug("%s: proxy client conn read event added.", __FUNCTION__);
							proxy_msg_t cdm;
							...
							if((ssize_t)sizeof(proxy_msg_t) == nwrit)
							{
								...
								{
									event_del(m_tcp);
									LC.notice("%s: proxy client is connected and running.", __FUNCTION__);
									return;
								}
								else
									LC.error("%s: comm client start failed.", __FUNCTION__);
								delete m_cc;
							}
							else
								LC.error("%s: client details write failed.", __FUNCTION__);
							event_del(m_conn_rd);
						}
						else
							LC.error("%s: conn-read event addition failed.", __FUNCTION__);
						event_free(m_conn_rd);
						m_conn_rd = NULL;
					}
					else
						LC.error("%s: conn-read event allocation failed.", __FUNCTION__);
					event_del(m_conn_wr);
				}
				else
					LC.error("%s: conn-write event addition failed.", __FUNCTION__);
				event_free(m_conn_wr);
				m_conn_wr = NULL;
    		}
			else
				LC.error("%s: conn-write event allocation failed.", __FUNCTION__);
    	...
    	else
    	{
            int errcode = errno;
            char errmsg[256];
            LC.error("%s: pipe() failed with error %d : [%s].",
            		__FUNCTION__, errcode, strerror_r(errcode, errmsg, 256));
    	}
    	close(m_conn_sock);
    	m_conn_sock = -1;
...
{
	LC.info("%s: accept timed out waiting for an incoming connection.", __FUNCTION__);
}
...
void cct_proxy_service::on_connwr1()
{
	...
	if(0 != event_add(m_conn_wr, NULL))
	{
        LC.error("%s: conn write 2 event add failed.", __FUNCTION__);
        exit(-__LINE__);
	}
}
...
void cct_proxy_service::on_connwr2()
{
	...
	if(0 > result)
	{
		int errcode = errno;
		char errmsg[512];
        LC.error("%s: splice() failed with error %d : [%s].",
        		__FUNCTION__, errcode, strerror_r(errcode, errmsg, 256));
        exit(-__LINE__);
	}
	else
	...
	if(0 != event_add(m_conn_wr, NULL))
	{
        LC.error("%s: conn write 1 event add failed.", __FUNCTION__);
        exit(-__LINE__);
	}
}
...
void cct_proxy_service::on_connrd()
{
	...
	if(0 != event_add(m_tcp, &minute))
	{
        LC.error("%s: accept event add failed.", __FUNCTION__);
        exit(-__LINE__);
	}
}
...
void cct_proxy_service::on_comm_message(const unsigned int src_id, const unsigned char * msg, const size_t size)
{
	...
	if((ssize_t)(sizeof(proxy_msg_t) + size) != nwrit)
	{
		if(0 > nwrit)
		{
			int errcode = errno;
			char errmsg[512];
	        LC.error("%s: writev() failed with error %d : [%s].",
	        		__FUNCTION__, errcode, strerror_r(errcode, errmsg, 256));
	        exit(-__LINE__);
		}
		else
		...
		LC.debug("%s: %lu bytes written to conn pipe.", __FUNCTION__, (size_t)nwrit);
}
...
void cct_proxy_service::update_peer_comm(const unsigned int party_id, const unsigned int connected)
{
	...
	if((ssize_t)sizeof(proxy_msg_t) != nwrit)
	{
		if(0 > nwrit)
		{
			...
			LC.error("%s: peer comm update written %lu out of %lu bytes.", __FUNCTION__, (size_t)nwrit, sizeof(proxy_msg_t));
	}
	else
		LC.debug("%s: %lu bytes of peer comm update written to conn pipe.", __FUNCTION__, (size_t)nwrit);
}
...
void cct_proxy_service::process_conn_msgs()
{
	while(!m_conn_data.empty())
	{
		...
		{
			LC.error("%s: invalid message type %u from proxy client conn.", __FUNCTION__, hdr.type);
			exit(-__LINE__);
		}
